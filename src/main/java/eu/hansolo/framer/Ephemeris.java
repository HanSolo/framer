...
public class Ephemeris {
    ...
    public static final double WGS84_a_in_km           = Constants.WGS84_a / 1000;
     ...
     * @param day    The day.
     * @param latitude Latitude for the observer.
     * @param longitude Longitude for the observer.
     *
     * @throws Exception If the date does not exists.
     */
    public Ephemeris(final int year, final int month, final int day, final double latitude, final double longitude) throws Exception{
        this(year, month, day, 0, 0, 0, latitude, longitude);
    }
    /**
     ...
     * @throws Exception If the date does not exists.
     */
    public Ephemeris(int year, int month, int day, int hour, int minute, int second, double latitude, double longitude) throws Exception {
        ...
        this.minute               = minute;
        this.second               = second;
        this.latitude             = longitude * DEG_TO_RAD;
        this.longitude            = latitude * DEG_TO_RAD;
     ...
     */
    public static int[] getDate(final double julianDay) throws Exception {
        if (julianDay < 2299160.0 && julianDay >= 2299150.0) {
            throw new Exception("invalid julian day " + julianDay + ". This date does not exist.");
        }
    ...
    public static ZonedDateTime getZonedDateTime(final double julianDay, final ZoneOffset zoneOffset) throws Exception {
        int[]  date           = getDate(julianDay);
        int    timeZoneOffset = zoneOffset.getTotalSeconds() / 3600;
    ...
    public static String getDateAsString(final double julianDay) throws Exception {
        return getDateAsString(julianDay, 0);
    }

    /**
     * Returns a date as a string.
     *
     * @param julianDay The Juliand day.
     *
     * @return The String.
     *
     * @throws Exception If the date does not exists.
     */
    public static String getDateAsString(final double julianDay, final int timezoneOffset) throws Exception {
        if (julianDay == -1) return "NO RISE/SET/TRANSIT FOR THIS OBSERVER/DATE";

        int[] date = Ephemeris.getDate(julianDay);
        return date[0] + "/" + date[1] + "/" + date[2] + " " + ((date[3] + timezoneOffset) % 24) + ":" + date[4] + ":" + date[5] + " UT";
    }
    ...
    public static double normalizeRadians(double rad) {
        if (rad < 0 && rad >= -TWO_PI) return rad + TWO_PI;
        if (rad >= TWO_PI && rad < FOUR_PI) return rad - TWO_PI;
        if (rad >= 0 && rad < TWO_PI) return rad;

        rad -= TWO_PI * Math.floor(rad * TWO_PI_INVERSE);
        if (rad < 0.) rad += TWO_PI;
     ...
     *
     * @param twilight The Twilight.
     */
    public void setTwilight(final TWILIGHT twilight) {
        this.twilight = twilight;
    }

    private void setUTDate(final double julianDay) {
        jd_UT = julianDay;
        t = (julianDay + ttMinusUT / SECONDS_PER_DAY - J2000) / JULIAN_DAYS_PER_CENTURY;
    }

    public void setLocation(final double latitude, final double longitude) throws Exception {
        this.sunriseZDT           = ZonedDateTime.now();
        this.sunsetZDT            = ZonedDateTime.now();
        this.sunriseGoldenHourZDT = ZonedDateTime.now();
        this.sunsetGoldenHourZDT  = ZonedDateTime.now();
        ...
        this.latitude             = longitude * DEG_TO_RAD;
        this.longitude            = latitude * DEG_TO_RAD;
        setDate(this.year, this.month, this.day, this.hour, this.minute, this.second);
    }

    public void setDate(final LocalDate localDate) throws Exception {
        setDate(localDate.getYear(), localDate.getMonthValue(), localDate.getDayOfMonth());
    }
    public void setDate(final LocalDateTime localDateTime) throws Exception {
        setDate(localDateTime.getYear(), localDateTime.getMonthValue(), localDateTime.getDayOfMonth(),
    ...
    private double[] getMoon() {
        ...
        return new double[]{ longitude, latitude, distance * WGS84_a_in_km / AU, Math.atan(1737.4 / (distance * WGS84_a_in_km)) };
    }

    private double[] doCalc(final double[] position) {
        ...
        position[0] *= DEG_TO_RAD;
        position[1] *= DEG_TO_RAD;
        double cl = Math.cos(position[1]);
        double x  = position[2] * Math.cos(position[0]) * cl;
        double y  = position[2] * Math.sin(position[0]) * cl;
        double z  = position[2] * Math.sin(position[1]);
        ...
        double radiusAU     = WGS84_a_in_km / AU;
        ...
        switch (twilight) {
            ...
            case HORIZON_34arcmin:
                ...
                // of the upper limb.
                tmp = -(34.0 / 60.0) * DEG_TO_RAD - position[3];
                break;
            case BLUE_HOUR:
                tmp = -4 * DEG_TO_RAD;
                break;
    ...
    public void calcEphemeris(final ZoneId zoneId) {
        try {
